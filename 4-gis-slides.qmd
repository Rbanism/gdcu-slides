...
author: "Clémentine Cottineau-Mugadza, Ignacio Urria Yáñez"
format: 
  revealjs:
...
-   organise your workspace in `RStudio`
...
-   Import OpenStreetMap (OSM) vector data using the API
...
- Within R, there is a corresponding package called [Leaflet](https://rstudio.github.io/leaflet/).

- As with `ggplot`, you build a leaflet map as a collection of layers: `leaflet(data) |> addTiles() |> addPolygons(~variable)`
...
![](fig/challengeZoom.png){fig-align="center"}
...
-   Use the `Nominatim` and `OverPass` API within R

-   Use the `osmdata` package to retrieve
    geospatial data
...
- Perform **geoprocessing operations** such as **union, join, and intersection** with dedicated functions from the sf package
...
If you were using pen&paper or a traditional GIS software, what would be your workflow?
...
We then need to join the aggregated number of centroids with the original layer, using a spatial left join. The corresponding function is `st_join(., left=T)`.
...
We aggregate them by ID number (`group_by(ID)`) and count how many centroids are contained in each polygon-buffer. Then we join the two data frames.
...
## Mapping the number of buildings with initial building map 
...
ggsave(
  filename = "fig/ConservationBrielle.png",
  plot = p
)
```
